...
vec4 effect(vec4 vertex_color, Image image, vec2 texture_coords, vec2 vertex_position)
{
    ...
    const vec3 black = vec3(0.95);

    const float bloom_magnitude = 0.6;
    float bloom = 1 + sin(elapsed * 2) * bloom_magnitude;
    float contrast_normalization = bloom / (1 + bloom_magnitude);
    ...
    return vec4(vec3(res) * contrast_normalization, 1);
}
