...
vec4 effect(vec4 vertex_color, Image image, vec2 texture_coords, vec2 vertex_position)
{
    if (mode == 0)  // initialize
    {
        float offset = 0.3;
        float scale = 100;
        float x = A + simplex_noise(vec3(texture_coords.xy * scale, A)) * offset;
        float y = B / A + simplex_noise(vec3(texture_coords.xy * scale, A)) * offset;
        return vec4(x, y, 0, 1);
    }
    else if (mode == 2) // draw
    {
        vec2 uv = Texel(image, texture_coords.xy).xy;
