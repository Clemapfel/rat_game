...
vec4 effect(vec4 vertex_color, Image image, vec2 texture_coords, vec2 fragment_position) {
    ...
    for (int i = 0; i < n_octaves; i++) {
        ...
        vec2 offset = vec2(cos(total * 2 * PI), sin(total * 2 * PI)) * amplitude * frequency * fraction * intensity;
        pos.xy += offset;
    }
    ...
    float eps = 0.15;
    float radius = (fraction) * (1 + 2 * eps) - eps;
    value = smoothstep(radius, radius + eps, dist * 2);

    return value * black + 2 * fwidth(value) * red * 2.1 * (1 - fraction);
}
