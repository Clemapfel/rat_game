...
function rt.Cloth:realize()
    ...
    for i = 1, self._n_nodes do
        ...
        local color = rt.hsva_to_rgba(rt.HSVA(self._grid_i_to_node_i[col_i][row_i] / self._n_nodes))
        ...
        if i < self._n_nodes and row_i ~= self._n_nodes_per_column then
            table.insert(self._pairs, i)
            table.insert(self._pairs, i + 1)
            self._n_pairs = self._n_pairs + 1
        end

        if i > self._n_nodes_per_column then
            table.insert(self._pairs, i)
            table.insert(self._pairs, i - self._n_nodes_per_column)
            self._n_pairs = self._n_pairs + 1
        end
...
function rt.Cloth:update(delta, n_iterations)
    ...
    for i = 1, n, 2 do
        ...
        positions[x_i] = current_x + (current_x - old_x) * friction + 1 * gravity_x * delta_squared
        positions[y_i] = current_y + (current_y - old_y) * friction + 1 * gravity_y * delta_squared
    ...
    for _ = 1, n_iterations do
        do
            local i = self._grid_i_to_node_i[4][1]
            positions[i] = x
            positions[i + 1] = y
