...
function rt.TextBox:update(delta)
    ...
    local all_entries_done = self._first_visible_entry < self._n_entries
    ...
    for i = self._first_visible_entry, self._n_entries do
        ...
        if is_previous_done then
            ...
            if n_lines_shown > self._max_n_lines then
                self._target_text_y_offset = self._target_text_y_offset + (new_n_lines_visible - entry.n_lines_visible ) * entry.line_height
            end
    ...
    if all_entries_done and self._current_text_y_offset >= self._target_text_y_offset and self._current_frame_h >= self._target_frame_h then
...
function rt.TextBox:set_show_history_mode_active(b)
    ...
    if b then
        ...
        self:_update_target_frame_h()
        self._position_target_value = _SHOWN
        self._first_visible_entry = self._n_entries
