rt.settings.battle.animations.global_status_gained = {
    duration = 2
}

--- @class bt.Animation.GLOBAL_STATUS_GAINED
bt.Animation.GLOBAL_STATUS_GAINED = meta.new_type("GLOBAL_STATUS_GAINED", rt.QueueableAnimation, function(target, status)
...
function bt.Animation.GLOBAL_STATUS_GAINED:start()
    self._label = rt.Label("<o>Gained: <b><mono>" .. self._status:get_name() .. "</mono></b></o>")
    ...
    local start_x, start_y = bounds.width * 0.75, bounds.height * 0.25 + label_h
    local finish_x, finish_y = bounds.width * 0.75, bounds.height * 0.75 + label_h
...
function bt.Animation.GLOBAL_STATUS_GAINED:update(delta)
    local duration = rt.settings.battle.animations.global_status_gained.duration
