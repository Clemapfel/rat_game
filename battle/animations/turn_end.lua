rt.settings.battle.animations.turn_end = {
    duration = 2
}

--- @class bt.Animation.TURN_END
bt.Animation.TURN_END = meta.new_type("TURN_END", rt.QueueableAnimation, function(target)
    return meta.new(bt.Animation.TURN_END, {
        _target = target,
...
function bt.Animation.TURN_END:start()
    ...
    self._tunnel_top:resize(x, y - 0.5 * tunnel_h - gradient_h, w, gradient_h)
    self._tunnel_bottom:resize(x, y + 0.5 * tunnel_h, w, gradient_h)
end

--- @override
function bt.Animation.TURN_END:update(delta)
    local duration = rt.settings.battle.animations.turn_end.duration
...
--- @override
function bt.Animation.TURN_END:finish()
    -- noop
end

--- @override
function bt.Animation.TURN_END:draw()
