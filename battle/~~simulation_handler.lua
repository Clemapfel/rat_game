...
function bt.BattleScene:create_simulation_environment()
    ...
    env.list_consumables = function(entity_proxy)
        bt.assert_arg("list_conusmables", entity_proxy, bt.EntityProxy)
        local out = {}
        local entity = _get_native(entity_proxy)
        for slot_i = 1, entity:get_n_consumable_slot() do
            local consumable = _state:entity_get_consumable(entity, slot_i)
            if consumable ~= nil then
                table.insert(out, bt.create_consumable_proxy(_scene, entity, consumable))
            end
        end
        return out
    end

    env.add_consumable = function(entity_proxy, consumable_id)
    ...
    env.has_status = function(entity_proxy, status_proxy)
        bt.assert_args("remove_status",
            entity_proxy, bt.EntityProxy,
            stauts_proxy, bt.StatusProxy
        )

        if meta.is_status_proxy(status_id) then
            status_id = _get_native(status_id):get_id()
        end

        local native_id = env[status_id]
        if native_id == nil then
            bt.error_function("In env.has_status: status with `" .. status_id .. "` does not exist")
            return nil
        end
    ...
    env.get_status = function(entity_proxy, status_id)
        ...
        local native_id = env[status_id]
        if native_id == nil then
            bt.error_function("In env.get_status: status with `" .. status_id .. "` does not exist")
            return nil
        end
