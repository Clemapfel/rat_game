...
function meta.assert_is_entity_interface(x)
    if not meta.is_entity_interface(x) then
        rt.error("In " .. debug.getinfo(2, "n").name .. ": Excpected `bt.EntityInterface`, got `" .. meta.typeof(x) .. "`")
    end
end
...
function meta.assert_is_status_interface(x)
    if not meta.is_status_interface(x) then
        rt.error("In " .. debug.getinfo(2, "n").name .. ": Excpected `bt.StatusInterface`, got `" .. meta.typeof(x) .. "`")
    end
end
