...
vec4 effect(vec4 vertex_color, Image image, vec2 texture_coords, vec2 fragment_position) {
    ...
    for (int i = 0; i < octaves; i++) {
        ...
        vec2 offset = vec2(cos(total * 2 * PI), sin(total * 2 * PI)) * amplitude * frequency * fraction * 0.002;
        pos.xy += offset;
        uv += offset * fraction;
    }
    ...
    black_value = smoothstep(radius, radius + eps, dist * 2 + eps);

    return black_value * black + 2 * fwidth(black_value) * red * 2 * (1 - fraction);
}
