...
uniform vec2 paletteResolution;
...
vec4 colourFromGreyscale(float colour)
{
    ...
    float minColour = (uvX * 6.0) + halfTexel;
    float maxColour = (uvX * 10.0) + halfTexel;
    float clampedX = mix(minColour, maxColour, colour);

    return Texel(paletteTexture, vec2(clampedX, (float(paletteIndex) - 0.5) / sizeY));
}
