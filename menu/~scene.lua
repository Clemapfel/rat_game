...
function mn.Scene:_handle_button_pressed(which)
    ...
    elseif which == rt.InputButton.X then
        self._shared_list_sort_mode = self._shared_list_sort_mode_order[self._shared_list_sort_mode]
        self._shared_move_list:set_sort_mode(self._shared_list_sort_mode)
        self._shared_equip_list:set_sort_mode(self._shared_list_sort_mode)
        self._shared_consumable_list:set_sort_mode(self._shared_list_sort_mode)
...
function mn.Scene:realize()
    ...
    for entity in values(self._state.entities) do
        ...
        for i = 1, #equip_list do
            page.equi_entity_tabsps_and_consumables:set_object(i, equip_list[i])
        end
...
function mn.Scene:size_allocate(x, y, width, height)
    ...
    self._control_indicator:fit_into(x + width - control_w - m, y + height - m - control_h, control_w, control_h)

    local portrait_w = 6 * m
    local portrait_x, portrait_y = x + m, y + m + control_h + m
