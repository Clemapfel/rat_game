...
// src: https://github.com/Nikaoto/subpixel/blob/master/subpixel_grad.frag
vec4 effect(vec4 color, sampler2D tex, vec2 uv, vec2 px)
{
    ...
    vec4 texel = color * textureGrad(tex, uv + f * texel_size, ddx, ddy);
    return vec4(mix(black.rgb, texel.rgb, value), texel.a);
}